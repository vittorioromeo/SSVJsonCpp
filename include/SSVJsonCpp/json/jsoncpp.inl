...
namespace Json
{
	...
	inline Reader::Reader() : errors_(), document_(), begin_(), end_(), current_(), lastValueEnd_(), lastValue_(), commentsBefore_(), features_(Features::all()), collectComments_() {}
	inline Reader::Reader(const Features& features) : errors_(), document_(), begin_(), end_(), current_(), lastValueEnd_(), lastValue_(), commentsBefore_(), features_(features), collectComments_() {}
	inline bool Reader::parse(const std::string& document, Value& root, bool collectComments)
	{
		document_ = document;
	...
	inline bool Reader::parse(const char* beginDoc, const char* endDoc, Value& root, bool collectComments)
	{
	...
	inline bool Reader::readValue()
	{
		...
		switch(token.type_)
		{
			...
			default: return addError("Syntax error: value, object or array expected.", token);
		}
		if(collectComments_)
		{
	...
	inline bool Reader::expectToken(TokenType type, Token& token, const char* message)
	{
		readToken(token);
	...
	inline bool Reader::readToken(Token& token)
	{
		...
		switch(c)
		{
			...
			case '"': token.type_ = tokenString; ok = readString(); break;
			case '/': token.type_ = tokenComment; ok = readComment(); break;
			case '0': case '1': case '2': case '3': case '4': case '5': case '6': case '7': case '8': case '9': case '-': token.type_ = tokenNumber; readNumber(); break;
			case 't': token.type_ = tokenTrue; ok = match("rue", 3); break;
			case 'f': token.type_ = tokenFalse; ok = match("alse", 4); break;
			case 'n': token.type_ = tokenNull; ok = match("ull", 3); break;
	...
	inline bool Reader::match(Location pattern, int patternLength)
	{
		...
		while(index--) if(current_[index] != pattern[index]) return false;
	...
	inline bool Reader::decodeUnicodeCodePoint(Token& token, Location& current, Location end, unsigned int& unicode)
	{
	...
	inline bool Reader::decodeUnicodeEscapeSequence(Token& token, Location& current, Location end, unsigned int& unicode)
	{
		if(end - current < 4) return addError("Bad unicode escape sequence in std::string: four digits expected.", token, current);
		unicode = 0;
	...
	inline bool Reader::addError(const std::string& message, Token& token, Location extra)
	{
		ErrorInfo info;
		info.token_ = token;
	...
	inline bool Reader::addErrorAndRecover(const std::string& message, Token& token, TokenType skipUntilToken)
	{
	...
	inline void Reader::getLocationLineAndColumn(Location location, int& line, int& column) const
	{
	...
	inline ValueConstIterator::ValueConstIterator(const Value::ObjectValues::iterator& current) : ValueIteratorBase(current) {}

	inline ValueConstIterator& ValueConstIterator::operator=(const ValueIteratorBase& other)
	{
		copy(other);
		return *this;
	}
	inline ValueIterator::ValueIterator() {}
	inline ValueIterator::ValueIterator(const Value::ObjectValues::iterator& current) : ValueIteratorBase(current) {}

	inline ValueIterator::ValueIterator(const ValueConstIterator& other) : ValueIteratorBase(other) {}
	inline ValueIterator::ValueIterator(const ValueIterator& other) : ValueIteratorBase(other) {}
	...
	inline static char* duplicateStringValue(const char* value, unsigned int length = unknown)
	{
		...
		JSON_ASSERT_MESSAGE(newString != nullptr, "Failed to allocate std::string value buffer");
	...
	inline Value::CZString::CZString(ArrayIndex index) : cstr_(nullptr), index_(index) {}
	...
	inline bool Value::isConvertibleTo(ValueType other) const
	{
		switch(other)
		{
			case nullValue: return(isNumeric() && asDouble() == 0.0) ||(type_ == booleanValue && value_.bool_ == false) ||(type_ == stringValue && asString() == "") ||
									(type_ == arrayValue && value_.map_->size() == 0) ||(type_ == objectValue && value_.map_->size() == 0) || type_ == nullValue;
			case intValue: return isInt() ||(type_ == realValue && InRange(value_.real_, minInt, maxInt)) || type_ == booleanValue || type_ == nullValue;
			case uintValue: return isUInt() ||(type_ == realValue && InRange(value_.real_, 0, maxUInt)) || type_ == booleanValue || type_ == nullValue;
			case realValue: return isNumeric() || type_ == booleanValue || type_ == nullValue;
			case booleanValue: return isNumeric() || type_ == booleanValue || type_ == nullValue;
			case stringValue: return isNumeric() || type_ == booleanValue || type_ == stringValue || type_ == nullValue;
	...
	inline void Value::resize(ArrayIndex newSize)
	{
		...
		if(newSize == 0) clear();
		else if(newSize > oldSize) (*this)[newSize - 1];
		else
		{
	...
	inline const Value& Value::operator[](const std::string& key) const { return(*this)[key.c_str()]; }
	inline Value& Value::operator[](const StaticString& key) { return resolveReference(key, true); }

	inline Value& Value::append(const Value& value) { return(*this)[size()] = value; }
	...
	inline Value Value::removeMember(const std::string& key) { return removeMember(key.c_str()); }
	...
	inline bool Value::isInt() const
	{
		switch(type_)
		{
			...
			case realValue: return value_.real_ >= minInt && value_.real_ <= maxInt && IsIntegral(value_.real_);
			default: break;
		}
		return false;
	}
	inline bool Value::isUInt() const
	{
		switch(type_)
		{
			case intValue: return value_.int_ >= 0 && LargestUInt(value_.int_) <= LargestUInt(maxUInt);
			case uintValue: return value_.uint_ <= maxUInt;
			case realValue: return value_.real_ >= 0 && value_.real_ <= maxUInt && IsIntegral(value_.real_);
			default: break;
		}
		return false;
	}
	...
	inline bool Value::isDouble() const { return type_ == realValue || isIntegral(); }
	...
	inline void Value::setComment(const std::string& comment, CommentPlacement placement) { setComment(comment.c_str(), placement); }
	inline bool Value::hasComment(CommentPlacement placement) const { return comments_ != nullptr && comments_[placement].comment_ != nullptr; }
	inline std::string Value::getComment(CommentPlacement placement) const { if(hasComment(placement)) return comments_[placement].comment_; return ""; }
	inline std::string Value::toStyledString() const { StyledWriter writer; return writer.write(*this); }
	inline Value::const_iterator Value::begin() const
	{
		switch(type_)
		{
			case arrayValue:
			case objectValue: if(value_.map_) return const_iterator(value_.map_->begin()); break;
			default: break;
		}
	...
	inline Value::const_iterator Value::end() const
	{
		...
		{
			case arrayValue:
			case objectValue: if(value_.map_) return const_iterator(value_.map_->end()); break;
			default: break;
		}
		return const_iterator();
	}
	inline Value::iterator Value::begin()
	{
		...
		{
			case arrayValue:
			case objectValue: if(value_.map_) return iterator(value_.map_->begin()); break;
			default: break;
		}
		return iterator();
	}
	inline Value::iterator Value::end()
	{
		...
		{
			case arrayValue:
			case objectValue: if(value_.map_) return iterator(value_.map_->end()); break;
			default: break;
		}
		return iterator();
	}
	...
	inline void Path::makePath(const std::string& path, const InArgs& in)
	{
		...
		while(current != end)
		{
			...
			{
				++current;
				if(*current == '%') addPathInArg(path, in, itInArg, PathArgument::kindIndex);
				else
				{
					ArrayIndex index = 0;
	...
	inline void Path::addPathInArg(const std::string&, const InArgs& in, InArgs::const_iterator& itInArg, PathArgument::Kind kind)
	{
		if(itInArg == in.end()) { }
	...
	inline const Value& Path::resolve(const Value& root) const
	{
		...
		for(Args::const_iterator it = args_.begin(); it != args_.end(); ++it)
		{
			...
			if(arg.kind_ == PathArgument::kindIndex) node = &((*node)[arg.index_]);
			else if(arg.kind_ == PathArgument::kindKey) node = &((*node)[arg.key_]);
		}
		return *node;
	}
	...
	inline std::string valueToString(double value)
	{
		...
		while(ch >= buffer)
		{
			switch(*ch)
			{
				case '0': case '1': case '2': case '3': case '4': case '5': case '6': case '7': case '8': case '9': --ch; continue;
	...
	inline std::string valueToString(bool value) { return value ? "true" : "false"; }
	inline std::string valueToQuotedString(const char* value)
	{
		if(value == nullptr) return "";
		if(strpbrk(value, "\"\\\b\f\n\r\t") == nullptr && !containsControlCharacter(value)) return std::string("\"") + value + "\"";
	...
	inline FastWriter::FastWriter() : yamlCompatiblityEnabled_(false), dropNullPlaceholders_(false) {}
	inline void FastWriter::enableYAMLCompatibility() { yamlCompatiblityEnabled_ = true; }
	inline void FastWriter::dropNullPlaceholders() { dropNullPlaceholders_ = true; }
	inline std::string FastWriter::write(const Value& root) { document_ = ""; writeValue(root); document_ += "\n"; return document_; }
	inline void FastWriter::writeValue(const Value& value)
	{
		switch(value.type())
		{
			case nullValue: if(!dropNullPlaceholders_) document_ += "null"; break;
			case intValue: document_ += valueToString(value.asLargestInt()); break;
			case uintValue: document_ += valueToString(value.asLargestUInt()); break;
			case realValue: document_ += valueToString(value.asDouble()); break;
			case stringValue: document_ += valueToQuotedString(value.asCString()); break;
	...
	inline StyledWriter::StyledWriter() : rightMargin_(74), indentSize_(3), addChildValues_() {}
	...
	inline void StyledWriter::writeValue(const Value& value)
	{
		switch(value.type())
		{
			...
			case stringValue: pushValue(valueToQuotedString(value.asCString())); break;
	...
	inline bool StyledWriter::hasCommentForValue(const Value& value) { return value.hasComment(commentBefore) || value.hasComment(commentAfterOnSameLine) || value.hasComment(commentAfter); }
	...
	inline StyledStreamWriter::StyledStreamWriter(std::string indentation) : document_(nullptr), rightMargin_(74), indentation_(indentation), addChildValues_() {}
		...
		else
		{
			...
			if(isArrayMultiLine)
			{
				...
				for(;;)
				{
					const Value& childValue = value[index];
					writeCommentBeforeValue(childValue);
					if(hasChildValue) writeWithIndent(childValues_[index]);
					else
					{
						writeIndent();
						writeValue(childValue);
	...
	inline void StyledStreamWriter::writeWithIndent(const std::string& value) { writeIndent(); *document_ << value; }
	...
	inline void StyledStreamWriter::writeCommentAfterValueOnSameLine(const Value& root)
	{
		if(root.hasComment(commentAfterOnSameLine)) *document_ << " " + normalizeEOL(root.getComment(commentAfterOnSameLine));
		if(root.hasComment(commentAfter))
		{
	...
	inline bool StyledStreamWriter::hasCommentForValue(const Value& value) { return value.hasComment(commentBefore) || value.hasComment(commentAfterOnSameLine) || value.hasComment(commentAfter); }
	...
	inline std::ostream& operator<<(std::ostream& sout, const Value& root) { Json::StyledStreamWriter writer; writer.write(sout, root); return sout; }
}
